...
const TeamSparkLine = ({ team }: TeamSparkLineProps) => {
  const finishedLastGames = team.last_matches
    ?.filter((game: ILastMatch) => game.status === 'finished')
    .slice(-3)

  // const lastGame = team.last_matches?.filter(game => game.status === 'finished').slice(-1)
  const futureGame = team.last_matches
    ?.filter((game: ILastMatch) => game.status === 'notstarted')
    .slice(0, 1)
  ...
  return (
    <div className={styles.team_sparkline_container} id="container">
      <div
        className={styles.team_spark_line_wrapper}
        onClick={() => null}
        style={{ borderBottom: border }}
      >
        <div className={`${styles.team_number} ${styles[theme]}`}>{team.id + 1}</div>
        ...
        <div className={styles.team_info} id="team-sparkline-name">
          <div className={`${styles?.team_name} ${styles[theme]}`}>
            {language === 'Eng' ? team?.team_short_name : team?.team_short_name_ru}
          </div>
        ...
        <div className={styles.last_games} id="last-games">
          ...
          {team.last_matches &&
            finishedLastGames.map((game: ILastMatch) =>
              !game?.is_home ? (
                <SparklineLastGame
                  key={game.uuid}
                  ...
                  team_goals={
                    game.is_home
                      ? game.score.short_score.full_time[0]
                      : game.score.short_score.full_time[1]
                  }
                  rival_team_goals={
                    game.is_home
                      ? game.score.short_score.full_time[1]
        ...
        {Array.isArray(futureGame) && futureGame.length > 0 && (
          <div className={styles.next_game} id="next-game">
            <NextGameSparkLine
              team_logo={
                futureGame[0]?.is_home ? futureGame[0]?.away_team.img : futureGame[0]?.home_team.img
              }
