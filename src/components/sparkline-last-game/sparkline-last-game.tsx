...
const SparklineLastGame = ({
  rival_team_goals,
  game_type,
  rival_team_logo,
  team_goals,
}: PreviousSparkLineGameTsProps) => {
  ...
  return (
    <div className={styles.previous_sparkline_game}>
      <div className={styles.game_info}>
        <div style={{ color: theme === 'dark' ? 'white' : '#333333' }}>
          {game_type ? homeGameType : awaygameType}
        </div>
        <div className={final_score}>
          <span>{team_goals}</span>:<span>{rival_team_goals}</span>
        </div>
      </div>
