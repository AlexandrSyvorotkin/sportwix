...
import styles from './last-game-item.module.scss'
...
const LastGameItem = ({ result, match_id }: RecentResultItemProps) => {
  ...
  function showElement(e: any) {
    ...
    const hidden = collapsible.style.visibility === 'hidden' || collapsible.style.visibility === ''
  ...
  const totalGoals = classNames({
    [styles.total_goals]: true,
    [styles.total_goals_light]: false,
  })
  ...
  return (
    <div className={styles.blockradius}>
      <div
        id="container"
        className={recentGamesContainerStyles}
        onMouseEnter={e => showElement(e)}
        onMouseLeave={e => hideElement(e)}
      >
        <div className={styles.title} onClick={() => null}>
          <div className={`${styles.recent_result_item}`}>
            <div className={styles.team} id={`recent_game${match_id}`}>
              <span className={styles.home_team_name}>{result.home_team.name_ru}</span>
              <div className={styles.team_logo}>
                <img src={`${result.home_team.img}`} alt="" />
              </div>
            </div>
            <div className={totalGoals}>
              <span>
                {result.status === 'finished' ? result?.score?.short_score?.full_time[0] : '-'}
              </span>
              <span>:</span>
              <span>
                {result.status === 'finished' ? result?.score?.short_score?.full_time[1] : '-'}
              </span>
            </div>
            <div className={styles.team}>
              <div className={styles.team_logo}>
                <img src={`${result.away_team.img}`} alt="" />
              </div>
              <span className={styles.away_team_name}>{result.away_team.name_ru}</span>
            </div>
          </div>
        </div>
