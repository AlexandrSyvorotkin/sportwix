  ...
  if (['1', '2', '3'].includes(place)) {
    return (
      <div className="flex items-center justify-center w-full">
        <div
          className={`${CUP_STYLES[place]} w-[40px] h-[40px] rounded-full flex items-center justify-center font-normal text-[14px] leading-[18px] border-solid`}
        >
          {children}
        </div>
      </div>
...
const TeamResultsTable = () => {
  const firstSelectedTeamSeasonResults = useAppSelector(
    (state: RootState) => state.tournamentSlice.firstSelectedTeam?.tabs?.seasons
  )
  const language = 'Eng'
      ...
      <div className="h-full w-4/5">
        <div className="flex flex-col w-full">
          <div className="w-full h-[50px] flex items-center">
            <div className="p-[15px] flex justify-center items-center w-1/7 text-center text-[14px]">
              {language === 'Eng' ? team_results_table.season.eng : team_results_table.season.ru}
            </div>
            <Separator className="w-[1px] h-full" />
            <div className="p-[10px] flex justify-center items-center w-1/7 text-center text-[14px]">
              {language === 'Eng' ? team_results_table.epl.eng : team_results_table.epl.ru}
            </div>
            <Separator className="w-[1px] h-full" />
            <div className="p-[10px] flex justify-center items-center w-1/7 text-center text-[14px]">
              {language === 'Eng' ? team_results_table.eng_cup.eng : team_results_table.eng_cup.ru}
            </div>
            <Separator className="w-[1px] h-full" />
            <div className="p-[10px] flex justify-center items-center w-1/7 text-center text-[14px]">
              {language === 'Eng'
                ? team_results_table.league_cup.eng
                : team_results_table.league_cup.ru}
            </div>
            <Separator className="w-[1px] h-full" />
            <div className="p-[10px] flex justify-center items-center w-1/7 text-center text-[14px]">
              {language === 'Eng' ? team_results_table.ucl.eng : team_results_table.ucl.ru}
            </div>
            <Separator className="w-[1px] h-full" />
            <div className="p-[10px] flex justify-center items-center w-1/7 text-center text-[14px]">
              {language === 'Eng' ? team_results_table.el.eng : team_results_table.el.ru}
            </div>
            <Separator className="w-[1px] h-full" />
            <div className="p-[10px] flex justify-center items-center w-1/7 text-center text-[14px]">
              {language === 'Eng' ? team_results_table.cs.eng : team_results_table.cs.ru}
            </div>
          </div>
          <Separator className="w-full h-[1px]" />
        </div>
