...
export function candleChart(
    ...
    constructor() {
      ...
      this.display = () => {
        ...
        if (this.p2.abs.x === undefined) {
          ...
          drawLine(this.p1.abs.x, this.p1.abs.y, this.temp.abs.x, this.temp.abs.y)
        } else {
          this.updatePointAbs(this.p1)
    ...
    constructor(speed, smooth) {
      ...
      this.scrolled = e => {
        ...
        if (e.deltaY) {
          ...
          let normalizedDelta = Math.round(
            ((Math.sign(e.deltaY) * camera.sizeY) / 100) * this.speed
          )
      ...
      this.update = () => {
        if (
          ...
          (chart.matchWidth > canvas.width / 3 && Math.sign(this.acceleration) < 0)
        ) {
          this.moving = false
          this.acceleration = 0
        } else {
          if (Math.abs(this.acceleration) > 1) {
            ...
            if (this.ctrlKey) {
              ...
              let currentRightChartPoint = Math.min(
                canvas.width - camera.right,
                canvas.width - chart.rightOffset
              )

              let rightChartRelToCameraX = (-camera.left + currentRightChartPoint) / camera.sizeX

              camera.left = Math.min(camera.limit, camera.left + delta * rightChartRelToCameraX)
              camera.right = Math.min(
                camera.limit,
                camera.right + delta * (1 - rightChartRelToCameraX)
              )
            }
  ...
  class ChartDrawer {
    ...
    draw() {
      ...
      for (let i = 0; i < this.candles.length; i++) {
        ...
        if (drawAttempt) {
          if (mouse.x > candlePosX - candleWidth / 2 && mouse.x < candlePosX + candleWidth / 2) {
      ...
      let bottomDelta = Math.round(
        canvas.height - this.minVisibleY - this.bottomAxisOffset - this.bottomChartOffset
      )
  ...
  class BkgDrawer {
    update() {
      ...
      this.scaleAmount = Math.max(
        1,
        Math.pow(2, Math.floor(Math.log2(Math.floor(this.scaleMinSizeY / chart.metricHeight))))
      )

      this.topScaleMetric = Math.ceil(chart.zeroPosY / chart.metricHeight)
      this.bottomScaleMetric = Math.ceil((canvas.height - chart.zeroPosY) / chart.metricHeight)
      let calculateScaleX = () => {
    ...
    drawSeasonsBars() {
      ...
      for (let i = 0; i < seasons.length; i++) {
        ...
        let drawSeasonBarRect = (text = '', color) => {
          ...
          this.drawRectWithText(actualText, 'white', 'center', 'middle', color, x, y, w, h)
        }
    ...
    drawAxisCornerOverlay() {
      ...
      let invOffY = h - chart.bottomAxisOffset
      ctx.fillRect(invOffX, invOffY, chart.rightOffset, chart.bottomAxisOffset)
    }
    ...
    drawLastVisibleCandleOnY() {
      ...
      let textY = Math.ceil(chart.zeroPosY - close * chart.metricHeight + textTopX / 2)
  ...
  function defineChartParams() {
    outerMaxMetric = Math.max(
      ...candles.map(item => Math.max(...[item.open, item.high, item.low, item.close]))
    )

    outerMinMetric = Math.min(
      ...candles.map(item => Math.min(...[item.open, item.high, item.low, item.close]))
    )

    outerScaleY = (canvas.height - 250) / (outerMaxMetric - outerMinMetric)
    outerScaleX = (canvas.width - 120) / (candles.length - 1)
    return [candles, outerScaleX, outerScaleY, outerMaxMetric, outerMinMetric]
  }
  ...
  function drawCandle(candle, scaleX, scaleY, pos) {
    ...
    if (candle.is_first_match === true) {
      ...
      function drawLeagueLogo() {
        if (teamLogo.ready) {
          ctx.drawImage(teamLogo, pos.x - 15, bottomY - 30, 30, 30)
        }
      }
    ...
    function drawFirstTeamImg(x, y) {
      if (firstImg.ready) {
        ctx.drawImage(firstImg, x, y, 20, 20)
      }
    }
    function drawSecondTeamImg(x, y) {
      if (secondImg.ready) {
        ctx.drawImage(secondImg, x, y, 20, 20)
      }
    }

    if (chartType === 1) {
      if (
        teams.second_team_name === candle.home_team ||
        teams.second_team_name === candle.away_team
      ) {
    ...
    if (chartType === 2) {
      if (
        teams.first_team_name === candle.home_team ||
        teams.first_team_name === candle.away_team
      ) {
