  ...
  reducers: {
    ...
    setCurrentInteractivePath: (state, action: PayloadAction<interactivePaths>) => {
      switch (action.payload) {
        case 'sparkline':
          state.interactiveRoadpap.currentInteractivePathStep =
            state.interactiveRoadpap.sparkline.step
          state.interactiveRoadpap.sparkline.isActive = true
          break
        case 'championshipTable':
          state.interactiveRoadpap.currentInteractivePathStep =
            state.interactiveRoadpap.championship.step
          state.interactiveRoadpap.championship.isActive = true
          break
        case 'informationTabs':
          state.interactiveRoadpap.currentInteractivePathStep =
            state.interactiveRoadpap.informationTabs.step
    ...
    setNextIneractiveStep: (state, action: PayloadAction<interactivePaths>) => {
      switch (action.payload) {
        case 'sparkline':
          state.interactiveRoadpap.sparkline.step += 1
          state.interactiveRoadpap.currentInteractivePathStep += 1
          break
        case 'championshipTable':
          state.interactiveRoadpap.championship.step += 1
          state.interactiveRoadpap.currentInteractivePathStep += 1
          break
        case 'informationTabs':
          state.interactiveRoadpap.informationTabs.step += 1
          state.interactiveRoadpap.currentInteractivePathStep += 1
      }
    },
...
export const {
  ...
  nextStep,
  prevStep,
  activateGuidelineMode,
  startRoadmap,
  disableRoadmap,
  setCurrentStepText,
  setMobileCurrentStepText,
  disableGuidelineMode,
  disableEducationOffer,
  enableEducationOffer,
  setCurrentInteractivePath,
  setNextIneractiveStep,
  disableInteractiveRoadmap,
} = guidelineRoadmapSlice.actions
